...
export function LoginPage() {
  ...
  // Redirect zur Keycloak-UI bei Passwortänderungszwang
  useEffect(() => {
    if (passwordChangeRequired && passwordChangeUrl) {
      window.location.href = passwordChangeUrl;
    }
  }, [passwordChangeRequired, passwordChangeUrl]);
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
          <form onSubmit={handleSubmit} noValidate aria-label="Anmeldeformular">
            ...
            {/* Passwortänderungszwang — kurze Info bevor Redirect */}
            {passwordChangeRequired && (
              <div
                role="status"
                aria-live="polite"
                className="mb-4 flex items-start gap-2 rounded-lg bg-yellow-50 border border-yellow-200 px-4 py-3 text-sm text-yellow-800"
              >
                <span aria-hidden="true" className="mt-0.5 shrink-0">🔑</span>
                <span>
                  Du wirst zur Passwortänderung weitergeleitet…
                </span>
              </div>
            )}
